...
```{r}
phone_prob <- c()
for(i in 1:max(logged_data$Number_of_people)){
  small_data <- filter(logged_data, Number_of_people == i)
  phone_prob_val <- sum(small_data$Checked_phone == "Yes")/length(small_data$Checked_phone)
  phone_prob <- c(phone_prob, phone_prob_val)
}
plot(phone_prob, names = c(1, 2, 3, 4), main = "Probablity someone checks\ntheir phone, given the number of\npeople riding with them", xlab = "Number of people", ylab = "Probability", type = "l")
```
